            ...
            try
            {
...

                {
                    ...
                    Task.Run(() =>
                    {
                        if (label4.Text != "")
                        {
                            String[] pairs = { label4.Text };
                            foreach (var pair in pairs)
                            {
                                int position = pair.IndexOf("T");
                                if (position < 0)
                                    continue;


                                string time = (pair.Substring(position + 1));
                                DateTime ukDateFormat;
                                string ukFormat = "HH:mm:ss";
                                DateTime.TryParseExact(time, ukFormat, CultureInfo.InvariantCulture, DateTimeStyles.None, out ukDateFormat);
                                DateTime test = ukDateFormat;
                                string ampm = ukDateFormat.ToString("htt");
                                string datecom = DateTime.Now.ToString();


                                TimeSpan diff1 = Convert.ToDateTime(datecom).Subtract(Convert.ToDateTime(time));
